...
#include <time.h>
#include <stdlib.h>
#include "local_lib/hacking.h"

#define DATAFILE "/home/sdksdk/docs/firmware/jonEricson/0x200/chance.data"
...
int main(int argc, char *argv[]) {
	...
	while(choice != 7) {
		...
		if((choice < 1) || (choice > 7))
			printf("\n[!!] The number %d an invalid selection.\n\n",choice);
		else if(choice < 4) {
			if(choice != last_game) {
				if(choice == 1)
			...
			printf("\nYour accounct has been reset with 100 credits.\n\n");
			player.credits = 100;
		}
	}
...
void register_new_player() {
	...
	printf("You have been giver %u credits.\n", player.credits);
}
...
void update_player_data() {
	...
	if(fd == -1)
		fatal("in update_player_dara() while opening file");
	read(fd, &read_uid, 4);
